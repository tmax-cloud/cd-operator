...
// ExposeControllerInterface controls webhook exposure configuration, using ingress and service
type ExposeControllerInterface interface {
	Start()
}
...
func (i *exposeIngressReconciler) reconcile(obj runtime.Object, exposeMode exposeType) error {
	...
	if len(ing.Status.LoadBalancer.Ingress) > 0 && ing.Status.LoadBalancer.Ingress[0].IP != "" {
		ing.Spec.Rules[0].Host = fmt.Sprintf("cd-webhook.%s.nip.io", ing.Status.LoadBalancer.Ingress[0].IP)
	}
