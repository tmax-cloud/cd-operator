...
func (h *webhookHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.k8sClient.Get(context.Background(), types.NamespacedName{Name: configName, Namespace: ns}, app); err != nil {
		_ = utils.RespondError(w, http.StatusBadRequest, fmt.Sprintf("req: %s, cannot get Application %s/%s", reqID, ns, configName))
