...
type handler struct {
	applicationHandler apiserver.APIHandler
}
...
func NewHandler(parent wrapper.RouterWrapper, cli client.Client, authCli *authorization.AuthorizationV1Client, logger logr.Logger) (apiserver.APIHandler, error) {
	...
	applicationHandler, err := applications.NewHandler(namespaceWrapper, cli, authCli, logger)
	if err != nil {
		return nil, err
	}
	handler.applicationHandler = applicationHandler
...
func (h *handler) versionHandler(w http.ResponseWriter, _ *http.Request) {
	...
	apiResourceList.APIResources = []metav1.APIResource{
		{
			Name:       fmt.Sprintf("%s/%s", cdv1.APIKindApplication, cdv1.ApplicationAPISync),
			Namespaced: true,
		},
	}
